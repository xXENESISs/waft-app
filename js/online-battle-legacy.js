...
async function resolveMultiplayerTurnFromServer(data) {
  ...
  const isPlayer1 = multiplayerPlayerSocketId === data.player1;

  const ownAction = isPlayer1 ? data.action1 : data.action2;
  const opponentAction = isPlayer1 ? data.action2 : data.action1;

  const ownPressureControl = isPlayer1 ? data.hornedLizardPressureControl1 : data.hornedLizardPressureControl2;
  const opponentPressureControl = isPlayer1 ? data.hornedLizardPressureControl2 : data.hornedLizardPressureControl1;

  const { player, enemy } = getBattleFighters();

  const ownPressureLabel = getHornedPressureControlOption(ownPressureControl)?.title || null;
  const opponentPressureLabel = getHornedPressureControlOption(opponentPressureControl)?.title || null;

  lastPlayerAction = ownPressureLabel
    ? ownPressureLabel + " + " + prettyActionLabel(ownAction, player)
    : prettyActionLabel(ownAction, player);
  lastEnemyAction = opponentPressureLabel
    ? opponentPressureLabel + " + " + prettyActionLabel(opponentAction, enemy)
    : prettyActionLabel(opponentAction, enemy);
...
async function playTurn(playerAction) {
  ...
  if (isMultiplayer) {
    ...
    const { player } = getBattleFighters();
    const pendingPressureControl = isHornedLizardFighter(player) ? pendingHornedPressureControl : null;
    const pendingPressureLabel = pendingPressureControl ? getPendingHornedPressureControlLabel() : null;

    if (pendingPressureControl) {
      clearPendingHornedPressureControl();
    }
    ...
    lastTurnOutcome = "Waiting";
    lastTurnSummaryLines = pendingPressureLabel
      ? ["Action selected with Pressure Control: " + pendingPressureLabel + ". Waiting for opponent..."]
      : ["Action selected. Waiting for opponent..."];
    ...
    socket.emit("playerAction", {
      ...
      hornedLizardPressureControl: pendingPressureControl
    });
